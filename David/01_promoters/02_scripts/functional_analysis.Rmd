...
notD = classes
notD$classD = NULL
aging.output = func(classes, all, aging, descriptions)
print(aging.output)
```
```{r run-devel}
devel.output = func(classes, all, devel, descriptions)
print(devel.output)
```

```{r run-reproductive}
repro.output = func(classes, all, repro, descriptions)
print(repro.output)
```

```{r run-immune}
immune.output = func(classes, all, immune, descriptions)
print(immune.output)
```
...
```{r run-metabolo}
metabolo.output = func(classes, all, metabolo, descriptions)
print(metabolo.output)
```
```{r run-locomotion}
locomotion.output = func(classes, all, locomotion, descriptions)
print(locomotion.output)
```
```{r run-behavior}
behavior.output = func(classes, all, behavior, descriptions)
print(behavior.output)
```
```{r run-txn}
txn_reg.output = func(classes, all, txn_reg, descriptions)
print(txn_reg.output)
```

```{r run-txn_repress}
txn_repress.output = func(classes, all, txn_repress, descriptions)
print(txn_repress.output)
```
```{r run-neuro}
neuro.output = func(classes, all, neuro, descriptions)
print(neuro.output)
```
