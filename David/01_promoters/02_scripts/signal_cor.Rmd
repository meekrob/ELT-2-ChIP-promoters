...
# model 2c: Elt-2 binding AND dependency AND signal AND intestine enrichment
mod.embryo2c = lm.combo(rlogc.embryo ~ LE.log_chip_signal_mean*LE_bound*embryo_int_exp, 
                       data = data)

mod.embryo2c$r.squared
...
p1 = ggplot(data, 
  ...
  stat_smooth(color="black", 
              method="lm", 
              formula = y~x) + 
  ggtitle(bquote("ChIP signal only," ~ R^2 == .(round(mod.embryo0$r.squared,3)))) +
...
p2 = ggplot(data,
  ...
  ggtitle(bquote("ChIP_signal and ELT-2 binding" ~ R^2 == .(round(mod.embryo1$r.squared,3))))+ scale_color_manual(values=okabe)+ guides(color="none")
...
p3 = ggplot(data,
  ...
  ylim(c(0,19)) + 
  ggtitle(bquote("ChIP_signal and intestine enrichment status" ~ R^2 == .(round(mod.embryo2$r.squared,3)))) + ylab("DESeq2 rlog counts") + scale_color_manual(values=okabe)+ guides(color="none")
...
p4 = ggplot(data,
  ...
  ggtitle(bquote("ChIP signal, ELT-2 binding and enrichment," 
                 ~ R^2 == .(round(mod.embryo3$r.squared,3))))+ scale_color_manual(values=okabe)+ guides(color="none")
...
p4a = ggplot(data,
  ...
  ylab("DESeq2 rlog counts") + 
  ggtitle(bquote("ChIP signal, ELT-2 binding and enrichment," 
                 ~ R^2 == .(round(mod.embryo2a$r.squared,3))))+ scale_color_manual(values=okabe)+ guides(color="none")
...
plot_grid(p1,p2,p3,p5, ncol=2, labels="AUTO"); # p4, 
