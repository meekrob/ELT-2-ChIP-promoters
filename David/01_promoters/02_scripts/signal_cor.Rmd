...
ggplot(data %>% mutate(elt2.binding = ifelse(LE_bound, "ELT-2 unbound", "ELT-2 bound")),
  ...
  ylab("DESeq2 rlog counts") + ggtitle(bquote("Expression explained by ChIP_signal and ELT-2 called peak" ~ R^2 == .(round(s.mod.embryo1$r.squared,3))))

ggplot(data,
       aes(x= LE.log_chip_signal_mean, y=rlogc.embryo)) + geom_point() + stat_smooth(method="lm", formula = y~x) + facet_wrap(~embryo_int_exp)+ ylim(c(0,19)) + ggtitle(bquote("Expression explained by ChIP_signal and intestine enrichment status" ~ R^2 == .(round(s.mod.embryo2$r.squared,3)))) + ylab("DESeq2 rlog counts") 
...
mod.L11 = lm(rlogc.L1 ~ L1.log_chip_signal_mean, data = data)
#summary(mod.L11)
...
ggplot(data %>% filter( L1_int_exp == "enriched"), 
       aes(x= L1.log_chip_signal_mean, y=rlogc.L1)) + geom_point() + stat_smooth(method="lm") + facet_wrap(~L1_bound)#+ coord_cartesian(xlim=c(5,10),ylim=c(0,20))
...
```{r}
data = alldata
LE_bound = ifelse(data$LE_bound, "called peak", "no called peak")
...
df$levels.combined = factor(df$levels.combined, levels=rev(c(
      "called peak, equal",
      "no called peak, equal",
        "called peak, enriched",
      "no called peak, enriched",
#      "called peak, depleted",
#      "no called peak, depleted",
      "called peak, ftr",
      "no called peak, ftr"
)))
...
df %<>% filter(levels.combined != "no called peak, depleted") %>% filter(levels.combined != "called peak, depleted")
...
G.LE = ggplot(df, aes(x=expression.level, y=levels.combined, fill=multCompLetters)) + geom_density_ridges2(scale=2, quantile_lines=TRUE,
                       quantile_fun=function(x,...) mean(x)) + ggtitle("ANOVA followed by Tukey Honest Significant Differences (multicomparison correction)", subtitle="Different colors: signif. difference in means at .01") + guides(fill=FALSE) + ylab("Expression/ELT-2 binding") + xlab("Embryo intestine expression level") 
G.LE
...
L1_bound = ifelse(data$L1_bound, "called peak", "no called peak")
...
L3_bound = ifelse(data$L3_bound, "called peak", "no called peak")
rlogc.L3 = data$rlogc.L3
L3_int_exp = data$L3_int_exp
L3.aov = aov(rlogc.L3 ~ L3_bound * L3_int_exp)
L3.hsd = TukeyHSD(L3.aov, conf.level = .999)
opar = par(mar=c(3,10,2,1)); plot(L3.hsd, las=2,cex.axis=.5); par(opar)
...
G.L3 = ggplot(df, aes(x=expression.level, 
  ...
  geom_density_ridges2(scale=2, quantile_lines=TRUE,
                       quantile_fun=function(x,...) mean(x))  + 
  ggtitle("ANOVA followed by Tukey Honest Significant Differences (multicomparison correction)", subtitle="Different colors: signif. difference in means at .01") +
  guides(fill=FALSE) + 
...
Ns = data.frame(group=names(tbl), val=c(tbl), x=18.5, y=1:6 + .25)
