...
df = data.frame(expression.FoldChange = logFC, 
                ...
                #levels.combined = din.status.description
                )
table(df$levels.combined)
...
ggqqplot(df %>% filter(L1_bound != "ELT2 BOUND" & levels.combined != "ELT2 BOUND, unchanged_ELT2_minus"), x="expression.FoldChange", color="levels.combined")

```


```{r subgroups-normality}
dfl = split(df, df$levels.combined)
lapply(dfl, function(x){
  qxy = qqnorm(x$expression.FoldChange)
  ggplot(as.data.frame(qxy), aes(x=x,y=y)) + geom_point() + ggtitle(unique(x$levels.combined))
})
```
