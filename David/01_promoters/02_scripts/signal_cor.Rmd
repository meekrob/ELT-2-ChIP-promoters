...
G.L1.dineen= ggplot(df %>% filter(!is.na(levels.combined)), 
                    aes(y=expression.FoldChange, 
                      x=levels.combined,fill=multCompLetters)) + 
  geom_violin() +
  stat_summary(fun="mean",
               geom = "crossbar", 
               width = 0.5) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + geom_hline(yintercept = 0)
...
dunn.matrix <- function(dunn.out)
{
  levs = str_trim(str_split_fixed(dunn.out$res$Comparison, "-", n = 2))
  levs.u = unique(c(levs))
  levs.mx = matrix(numeric(length(levs.u)^2), nrow = length(levs.u))
  rownames(levs.mx) <- levs.u
  colnames(levs.mx) <- levs.u
  dunn.out$res$from = levs[ seq(1, length(levs), by = 2) ]
  dunn.out$res$to = levs[ seq(2, length(levs), by = 2) ]  
  
  apply(dunn.out$res, 1, function(x) {
    levs.mx[x['from'], x['to']] = as.numeric(x['P.adj'])
    levs.mx[x['to'], x['from']] = as.numeric(x['P.adj'])
    print(levs.mx)
    })
  levs.mx
}
...
# passing .01
dunn.out$res %>% filter(P.adj < .01) %>% select(Comparison)

# not passing .01 
dunn.out$res %>% filter(P.adj > .01) %>% select(Comparison, P.adj)
```
...
ggqqplot(df, x="expression.FoldChange") + facet_wrap(~L1_bound+din.status.description) + ggtitle("Normality (qqplots) of different subsets") +
  ...
  geom_vline(xintercept=0, color="light blue") + 
  ylab("Expression Fold Change in Intestine") +
