...
```{r btw-stage-use-gfp-minus-as-control, eval=FALSE}
cts.GFPplusMinus = cts %>% as.data.frame() %>% dplyr::select(contains('GFP')) %>% as.matrix() 
coldata.GFPplusMinus = coldata %>% filter(grepl("GFP", sample)) %>% mutate(group=ifelse(grepl('plus',group), 
                    'GFPplus', 'GFPminus'))
...
```{r use-whole-as-control}
cts.GFPplusMinus = cts %>% as.data.frame() %>% dplyr::select(contains('GFPplus'),contains('whole')) %>% as.matrix() 
coldata.GFPplusMinus = coldata %>% filter(grepl("GFPplus|whole", sample)) %>% mutate(group=ifelse(grepl('plus',group), 
                    'GFPplus', 'whole')) 
coldata.GFPplusMinus = coldata.GFPplusMinus[colnames(cts.GFPplusMinus),]
...
```{r perform-DE}
design(dds.GFPplusMinus) <- ~ group + group:stage
...
control_L1_v_embryo = results(dds.GFPplusMinus, contrast = list("groupGFPplus.stageL1"), lfcThreshold=1, alpha=.01)
...
control_L3_v_embryo = results(dds.GFPplusMinus, contrast = list("groupGFPplus.stageL3"), lfcThreshold=1, alpha=.01)
...
control_L3_v_L1 = results(dds.GFPplusMinus, contrast = list("groupGFPplus.stageL3", "groupGFPplus.stageL1"), lfcThreshold=1, alpha=.01)
...
ggplot(aes(x = log10(baseMean), y = log2FoldChange, color=tissue2, alpha=isSig,group=grp)) +
  ...
  #scale_color_manual(values=c('black','red')) +
  scale_alpha_manual(values=c(.05,.5)) + geom_hline(yintercept = 0)
