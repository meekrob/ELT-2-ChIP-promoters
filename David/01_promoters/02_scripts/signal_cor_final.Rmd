...
p3=ggplot(data %>% filter(!is.na(din.status.description)),
  ...
  theme_bw() +
  guides(color="none")
pdf(file="plots/signal_cor.xy.L1.dineen.status.pdf", height=5,width=12, family="ArialMT")
p3
dev.off()
...
down.t = t.test(rlogc.L1 ~ elt2.binding, data = data.L1 %>% filter(!is.na(din.status.description) & din.status.description == "down_ELT2_minus"), conf.level = 1-(.05/3) )

up.t = t.test(rlogc.L1 ~ elt2.binding, data = data.L1 %>% filter(!is.na(din.status.description) & din.status.description == "up_ELT2_minus"), conf.level = 1-(.05/3) )

unchanged.t = t.test(rlogc.L1 ~ elt2.binding, data = data.L1 %>% filter(!is.na(din.status.description) & din.status.description == "unchanged_ELT2_minus"), conf.level = 1-(.05/3) )


# this part is bullshit because geom_text(parse=T) reformats the %.2e into %.f
up.formatted.exp = floor(log(up.t$p.value,10))
down.formatted.exp = floor(log(down.t$p.value,10))
up.formatted.coef = round( up.t$p.value * 10 ^ -up.formatted.exp, 1)
down.formatted.coef = round( down.t$p.value * 10 ^ -down.formatted.exp, 1)
...
annotation_text = data.frame(elt2.binding=1.5,
                             ...
                             lab = c(
                               sprintf(fmt_string, down.formatted.coef,
                                       down.formatted.exp),
                               sprintf("italic(p) == %.2f", unchanged.t$p.value),
...
p = ggviolin(data.L1 %>% filter(!is.na(din.status.description)),
    ...
    panel.labs=list(din.status.description=c("down", 
                                             "unchanged", 
                                             "up"))) + 
  ggtitle("A) Higher mean expression of ELT-2 bound, ELT-2-dependant promoters") +
  ...
  geom_text(data = annotation_text, 
    ...
    geom_line(data= rbind(
            ...
            mapping=aes(x=x,y=y))
p
pdf(file="plots/signal_cor.violin.L1.dineen.status.pdf", height=3.2,width=8.4, family="ArialMT")
p
dev.off()
...
```{r convenient-place-to-output-stage-violins}
...
# this part is bullshit but it will match the x10^exp in the L1 sectoni
embryo.formatted.exp = floor(log(embryo.t$p.value,10))
embryo.formatted.coef = round( embryo.t$p.value * 10 ^ -embryo.formatted.exp, 1)

L1.formatted.exp = floor(log(L1.t$p.value,10))
L1.formatted.coef = round( L1.t$p.value * 10 ^ -L1.formatted.exp, 1)

L3.formatted.exp = floor(log(L3.t$p.value,10))
L3.formatted.coef = round( L3.t$p.value * 10 ^ -L3.formatted.exp, 1)
...
pdf(file="plots/signal_cor.violin.stages.pdf", height=4,width=8.4, family="ArialMT")
p
dev.off()         
```
...
p2=ggplot(data %>% mutate(stage=forcats::fct_relevel(stage, c("embryo","L1","L3"))),
  ...
  guides(color="none") + theme_bw() + theme(
    ...
    plot.title = element_text(face="bold"))
p2
pdf(file="plots/signal_cor.xy.stage.final.pdf",height=5,width=8, family="ArialMT")
p2
dev.off()
