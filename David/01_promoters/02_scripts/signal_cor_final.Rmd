...
annotation_text = data.frame(elt2.binding="ELT-2 unbound",
                             rlogc.L1 = 20,
                             lab = c(sprintf("p = %.2e", down.t$p.value),
                                             sprintf("p = %.2f", unchanged.t$p.value),
                                             sprintf("p = %.2e", up.t$p.value)),
...
p = ggviolin(data.L1 %>% filter(!is.na(din.status.description)),
    ...
    panel.labs=list(din.status.description=c("down", 
                                             "unchanged", 
                                             "up"))) + 
  ggtitle("Expression level as a function ELT-2 binding status", subtitle="panels indicate direction of change upon ELT-2 deletion, p-value is for t-test of no difference between bound and unbound") +
