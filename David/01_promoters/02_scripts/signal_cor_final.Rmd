...
annotation_text = data.frame(elt2.binding=1.5,
                             rlogc.L1 = c(21,21,22),
                             lab = c(
                               sprintf("italic(p) == %.1e", down.t$p.value),
                               sprintf("italic(p) == %.2f", unchanged.t$p.value),
                               sprintf("italic(p) == %.1e", up.t$p.value)),
...
p = ggviolin(data.L1 %>% filter(!is.na(din.status.description)),
    ...
    panel.labs=list(din.status.description=c("down", 
                                             "unchanged", 
                                             "up"))) + 
  ggtitle("Expression level as a function ELT-2 binding status, ELT-2 dependency (L1)") +
  xlab("") + ylab("Expression level (rlog counts)") +
  theme(
    axis.text.x = element_text(angle = 90, colour="black",vjust=.5),
...
pdf(file="plots/signal_cor.violin.L1.dineen.status.pdf", height=5,width=12, family="ArialMT")
p
dev.off()
...
annotation_text = data.frame(ELT2bound = FALSE,
                             rlogc = c(22,22,21),
                             lab = c(sprintf("italic(p) == %.1e", embryo.t$p.value),
                                   sprintf("italic(p) == %.1e", L1.t$p.value),
                                   sprintf("italic(p) == %.1e", L3.t$p.value)),
                             stage = factor(c("embryo", "L1", "L3"))
                             )

ggviolin(data=data, x="ELT2bound",y="rlogc",   
        add = c("mean_ci"),
#        draw_quantiles = c(.5),
     add.params = list(size=.1,
                         width=.75,
    ...
    ggtheme=theme_bw(),
    facet.by = "stage") + 
  ggtitle("Expression level as a function ELT-2 binding status") +
  xlab("") + ylab("Expression level (rlog counts)") +
#  theme_classic() +
  theme(axis.text.x=element_text(colour="black", angle=90,vjust=.5),
        axis.text.y=element_text(colour="black"),
        strip.background = element_blank(),
...
p2=ggplot(data %>% mutate(stage=forcats::fct_relevel(stage, c("embryo","L1","L3"))),
  ...
  guides(color="none") + theme_bw()
p2
pdf(file="plots/signal_cor.xy.stage.final.pdf",height=5,width=12, family="ArialMT")
p2
dev.off()
...
ggplot(data %>% mutate(stage=forcats::fct_relevel(stage, c("embryo","L1","L3"))),
  ...
  guides(color="none") + theme_bw() + theme(text = element_text(family = "Arial"))      
```
