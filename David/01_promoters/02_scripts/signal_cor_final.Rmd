...
plot_grid(title, p1, p2, p3, ncol=1,labels=c('','A','B','C'), rel_heights = c(0.1, 1, 1, 1));  ggsave("plots/signal_cor.xy.embryo.pdf", height=15,width=12)
...
```{r get-embryo-set}
data = alldata %>% dplyr::filter(embryo_int_exp %in% c('enriched','equal') & LE_bound &  din.status.description %in% c("down_ELT2_minus", "up_ELT2_minus")) %>% select(WBGeneID, LE.log_chip_signal_mean, rlogc.embryo, din.status.description, embryo_int_exp, LE_bound, wikigene_name)
...
```{r L1-r-squared}
data = alldata %>% filter(L1_int_exp %in% c('enriched','equal'))
#data %<>% filter(WBGeneID != 'WBGene00016153') # negative signal, lowest point
#data %<>% filter(WBGeneID != 'WBGene00012762') # kind of hot region, highest point in "depleted"
...
```{r L3-r-squared}
data = alldata %>% filter(L3_int_exp %in% c('enriched','equal'))
#data %<>% filter(WBGeneID != 'WBGene00016153') # negative signal, lowest point
#data %<>% filter(WBGeneID != 'WBGene00012762') # kind of hot region, highest point in "depleted"
