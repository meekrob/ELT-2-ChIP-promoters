...
idr.nonlog = as.data.frame(gr.promoters) %>% filter(is.finite(IDR_value)) %>% select('IDR_value','IDR_mean','IDR_max','IDR_sum') %>% gather(key="dataset") 
...
```{r runGO, message=F}
CLASSA_GO_RDS = file.path("topGO_saved_objects",stage, "classA.rds")
if (file.exists(CLASSA_GO_RDS)) {
  WORMGO=readRDS(CLASSA_GO_RDS)
}else{
  classA.go = runGO(genes[classA], genes, WORMGO)
  saveRDS(classA.go, CLASSA_GO_RDS)
} 

CLASSB_GO_RDS = file.path("topGO_saved_objects",stage, "classB.rds")
if (file.exists(CLASSB_GO_RDS)) {
  WORMGO=readRDS(CLASSB_GO_RDS)
}else{
  classB.go = runGO(genes[classB], genes, WORMGO)
  saveRDS(classB.go, CLASSB_GO_RDS)
} 

CLASSC_GO_RDS = file.path("topGO_saved_objects",stage, "classC.rds")
if (file.exists(CLASSC_GO_RDS)) {
  WORMGO=readRDS(CLASSC_GO_RDS)
}else{
  classC.go = runGO(genes[classC], genes, WORMGO)
  saveRDS(classC.go, CLASSC_GO_RDS)
} 
CLASSD_GO_RDS = file.path("topGO_saved_objects",stage, "classD.rds")
if (file.exists(CLASSD_GO_RDS)) {
  WORMGO=readRDS(CLASSD_GO_RDS)
}else{
...
classA.go$BP.result %>% filter(GO.ID=='GO:0045087') %>% mutate(class='A',stage=params$stage) %>% 
  ...
  rbind(classD.go$BP.result %>% filter(GO.ID=='GO:0045087') %>% mutate(class='D',stage=params$stage)) 
...
classA.go$BP.result %>% filter(GO.ID=='GO:0009792') %>% mutate(class='A',stage=params$stage) %>% 
  ...
  rbind(classD.go$BP.result %>% filter(GO.ID=='GO:0009792') %>% mutate(class='D',stage=params$stage)) 
...
classA.go$BP.result %>% filter(GO.ID=='GO:0008340') %>% mutate(class='A',stage=params$stage) %>% 
  ...
  rbind(classD.go$BP.result %>% filter(GO.ID=='GO:0008340') %>% mutate(class='D',stage=params$stage)) 
...
classA.go$BP.result %>% filter(GO.ID==go.term) %>% mutate(class='A',stage=params$stage) %>% 
  ...
  rbind(classD.go$BP.result %>% filter(GO.ID==go.term) %>% mutate(class='D',stage=params$stage)) 
...
classA.go$MF.result %>% filter(GO.ID==go.term) %>% mutate(class='A',stage=params$stage) %>% 
  ...
  rbind(classD.go$MF.result %>% filter(GO.ID==go.term) %>% mutate(class='D',stage=params$stage)) 
...
GOresults = list(classA=classA.go,classB=classB.go,classC=classC.go,classD=classD.go)
...
for (cls in c('classA', 'classB', 'classC', 'classD')){
  for(sect in c('BP','MF','CC')) {
    kabs[[cls]][[sect]] %>% 
      ...
      kable( label = cls, caption = sprintf("%s/%s", cls, sect), digits = 2) -> k
      print(k)
