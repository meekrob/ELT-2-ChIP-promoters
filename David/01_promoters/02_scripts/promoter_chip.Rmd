...
thresholds = seq(min(dataset$IDR_logTEN_max), max(dataset$IDR_logTEN_max), length.out = 50)
ROC.IDR.max = ROC(dataset$IDR_logTEN_max, dataset$class == 'classA')

data.frame(IDR_max=thresholds, proportion_of_classA=ROC.IDR.max$TPR(thresholds), proportion_of_classB=ROC.IDR.max$FPR(thresholds)) %>%
  ggplot(aes(x=IDR_max,y=proportion_of_classA)) + geom_line() +
  geom_line(aes(y=proportion_of_classB),color="red") +
  geom_segment(x=thresholds[17],xend=thresholds[17],
               y=TPR(thresholds[17]),yend=FPR(thresholds[17]))
```
