...
# TO DO
OUTPUT UNSHRUNK RES WITH ALTHYP LABELS HERE
...
```{r}
res_to_df(res_embryoGFPplus_vs_embryoGFPminus_ashr) %>% left_join(embryo_intestine_gene_cats, by = "WBGeneID") %>% drop_na(intestine_expression) %>%
  ggplot(aes(x =  log10(baseMean), y =log2FoldChange)) +
  geom_point(data = res_to_df(res_embryoGFPplus_vs_embryoGFPminus_ashr) %>% filter(!(WBGeneID %in%embryo_intestine_gene_cats$WBGeneID)),
             shape = 16, alpha = 0.5, stroke = 0, size = 2, color = "grey") +
  geom_point(shape = 16, alpha = 0.5, stroke = 0, size = 2, aes(color = intestine_expression)) +
  theme_bw() +
