...
sig = 0.01
stage = "L1"
rlog_status <- function(thresh, sig, stage, res){
all_samples_rld_df %>% 
  ...
  group_by(WBGeneID) %>%
  summarise(mean.rlog.counts = mean(rlog_counts), var.rlog.counts = var(rlog_counts)) %>%
  left_join(as.data.frame(res) %>% rownames_to_column(var = "WBGeneID"), by = "WBGeneID") %>%
  mutate(status = case_when(
    log2FoldChange > thresh & padj < sig ~ "enriched",
    log2FoldChange < thresh & padj < sig ~ "depleted",
    TRUE ~ "no_sig_diff",
...
```{r}
embryo_rlog_status_df <- rlog_status(thresh = thresh, sig = sig, stage = "embryo", res = res_embryoGFPplus_vs_embryoGFPminus)
head(embryo_rlog_status_df)
# write_csv(embryo_rlog_status_df, file = "../03_output/embryo_GFPplus_rlog_counts_status_df.csv", col_names = TRUE)
embryo_rlog_status_df <- read_csv(file = "../03_output/embryo_GFPplus_rlog_counts_status_df.csv", col_names = TRUE)
```
```{r}
L1_rlog_status_df <- rlog_status(thresh = thresh, sig = sig, stage = "L1", res = res_L1GFPplus_vs_L1_GFPminus)
head(L1_rlog_status_df)
# write_csv(L1_rlog_status_df, file = "../03_output/L1_GFPplus_rlog_counts_status_df.csv", col_names = TRUE)
```
```{r}
L3_rlog_status_df <- rlog_status(thresh = thresh, sig = sig, stage = "L3", res = res_L3GFPplus_vs_L3_GFPminus)
...
intestine_enriched_genes <- data.frame(embryo_rlog_status_df, stage = "embryo") %>% bind_rows(data.frame(L1_rlog_status_df, stage = "L1"), data.frame(L3_rlog_status_df, stage = "L3")) %>%filter(status == "enriched") %>% select(WBGeneID, stage)
...
```{r}
gfpplus_matrix <- all_samples_rld_df %>% select(WBGeneID, contains("GFPplus")) %>% filter(WBGeneID %in% c(embryo_rlog_status_df[embryo_rlog_status_df$status == "enriched",]$WBGeneID, L1_rlog_status_df[L1_rlog_status_df$status == "enriched",]$WBGeneID, L3_rlog_status_df[L3_rlog_status_df$status == "enriched",]$WBGeneID)) %>% column_to_rownames(var = "WBGeneID") %>% as.matrix()
```

```{r}
hist(rowMeans(gfpplus_matrix), main = "GFPplus sample rlog row means")
abline(v = mean_thresh, col = "red", lty = 2)
```


```{r}
hist(rowSds(gfpplus_matrix), main = "GFPplus sample rlog row standard deviations")
...
pheatmap(gfpplus_matrix_row_zscore[rowSds(gfpplus_matrix) >= sd_thresh & rowMeans(gfpplus_matrix) >= mean_thresh,], 
         cluster_cols = TRUE, 
         show_rownames = FALSE, 
         main = paste0("Intestine Development Clustered Heatmap\n thresholds: row mean >= ", mean_thresh, ", row SD >= ", sd_thresh))
```
