...
for(i in unique(q1_contingency_df$stage)){
  # print(i)
  q1_pval <- chisq.test(q1_contingency_df %>% filter(stage == i) %>% select(-stage, -intestine_expression))$p.value
  print(c(i, q1_pval))
  q1_pval_df <-q1_pval_df %>% bind_rows(data.frame(stage = i, chi.pval = q1_pval))
}
...
q1_table <- q1_contingency_df %>% 
  ...
  left_join(q1_pval_df %>% mutate(chi.padj = p.adjust(chi.pval, method = "bonferroni")), by = "stage")
