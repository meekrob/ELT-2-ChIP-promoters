...
for(i in unique(q1_contingency_df$stage)){
  # print(i)
  pval <- chisq.test(q1_contingency_df %>% filter(stage == i) %>% select(-stage, -intestine_expression))$p.value
  print(c(i, pval))
}
...
for(i in unique(q2_contingency_df$stage)){
  ...
  for(j in c("bound", "not_bound")){
    pval <- chisq.test(q2_contingency_df %>% filter(stage == i, promoter_status == j) %>% select(-(stage:promoter_status)))$p.value
    print(c(i, j,pval))
  }
}
