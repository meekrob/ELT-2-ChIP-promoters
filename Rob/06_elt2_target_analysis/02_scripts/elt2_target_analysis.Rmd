...
stopifnot(basename(dirname(getwd())) == "06_elt2_target_analysis")
stopifnot(basename(dirname(dirname(getwd()))) == "Rob")
stopifnot(basename(dirname(dirname(dirname(getwd())))) == "ELT-2-ChIP-revision")
david_dir = normalizePath("../../../david")
david_dir_plots = file.path(david_dir,"01_promoters/03_output/plots")
david_dir_plots
stopifnot(file.exists(david_dir_plots))
```
...
all_stages_chip  %>% left_join(transcript_type, by = "WBGeneID") %>% distinct(WBGeneID, .keep_all = TRUE) %>% group_by(biotype, intestine_expression) %>% summarise(n())
```
...
q1_table <- q1_contingency_df %>% 
  rowwise() %>%
  mutate(percent_bound = (100*bound)/(bound+not_bound),
...
q2_table <- q2_contingency_df %>% 
  rowwise() %>%
  mutate(percent_activated = (100*activated)/(activated+repressed+independent),
...
```{r message = FALSE}
# TF list
wtf3 <- read_csv("../../03_emb_L1_L3_intestine_RNAseq/01_input/TF3-0_namesonly.csv") %>% filter(!grepl("mex", Public_name))
